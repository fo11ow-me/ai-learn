"""调用追踪基础设施：模型/工具层的日志需带 task_id 归属，但通用服务层不感知任务。
...
def task_id_kv() -> str:
    """从上下文取 task_id 拼成 key=value 前缀；未设置返回空串。

    统一拼进模型/工具层日志，与链路事件的 task_id= 字段格式一致，按 task_id 可聚合
    单次执行的全部调用。
    """
...
def truncate_for_log(text: str, limit: int = TRACE_TRUNCATE_LIMIT) -> str:
    ...
    Prompt/模型输出可达数千字符，整段输出会刷屏且单行过长；截断必须带标记，读者知道
    内容被截断过。
    """
