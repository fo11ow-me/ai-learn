"""出题任务流水线（方案文档 3.2/5.1 三段式 + 联网搜索增强）：
检索资料（可选，失败自动降级）→ 生成题库 → 敏感词检查 → 写入任务状态"""
import asyncio
import logging
import re
import time
...
async def run_quiz_task(
    ...
    """后台执行出题任务：置 running → 检索资料（可选，失败自动降级）→ 生成（带整体超时）→ 合规检查 → completed；
    ...
    _logger.info("quiz task start task_id=%s content_len=%d", task_id, len(content))
    try:
        async with asyncio.timeout(settings.task_timeout_seconds):
            search_results = await _retrieve_search_materials(task_id, content, llm, settings, search)
            _logger.info("quiz llm start task_id=%s search_chars=%s", task_id, len(search_results) if search_results else "-")
            quiz: QuizSchema = await llm.generate_quiz(content, search_results=search_results)
...
async def _retrieve_search_materials(
    task_id: str, content: str, llm: LLMClient, settings: Settings, search: SearchClient | None
) -> str | None:
    ...
    try:
        async with asyncio.timeout(SEARCH_BUDGET_SECONDS):
            if is_url(content):
                ...
                return text or None
            plan: SearchPlanSchema = await llm.plan_search(content)
            _logger.info(
