...
### <a name='RAG'></a>5.6 私有知识库（RAG）出题设计

> 目标（对应需求 P1「RAG 私有知识库」）：用户上传私有文档（企业培训资料、客服话术、内部题库等），AI 出题优先基于这些资料；指定知识库时严格模式**永不联网**。已落地：多库管理 + 上传解析 + 向量检索 + 判定补全决策矩阵。

**检索架构（D2）**：MySQL 为**权威源**（文档全文存 `content_text`），Chroma 为**可重建派生索引**（单 collection `kb_chunks`，metadata 按 `user_id`/`kb_id`/`doc_id` 过滤隔离，不按用户分 collection——避免 collection 数量膨胀）。向量模型 `qwen3.7-text-embedding`（阿里云百炼 OpenAI 兼容端点，1024 维），cosine 空间，`relevance = 1 - distance`，低于 `KB_MIN_SCORE`（默认 0.3）的片段硬过滤丢弃。分块用 RecursiveCharacterTextSplitter（chunk_size=500 / overlap=50，中文按字符计数）。

**数据表（D1）**：

| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `knowledge_bases` | user_id / name / description | 每个用户可建多库；同用户重名 409 |
| `knowledge_documents` | knowledge_base_id / user_id / filename / file_type / status / chunk_count / content_text | 全文落库（解析即同步完成）；状态机 `uploading → ready / failed`；敏感词命中落库 `failed` 且不参与出题 |

**写顺序约束**：先 MySQL（解析全文落库）后 Chroma（后台任务向量化）——Chroma 失败只标记 `failed` 可重试/重建，MySQL 全文不丢；删除先删 Chroma 索引后删记录，避免幽灵向量。覆盖语义：同库同名上传 = 更新既有文档（doc_id 不变，旧 chunk 删除后重写）；`reindex` 按 MySQL 全文幂等重建（Chroma 卷丢失可恢复）。

**文档解析**：PDF（PyMuPDF 文本层，扫描件/无文本层明确拒绝 `PARSE_FAILED`——不做 OCR）、Word（python-docx，表格转「列1 | 列2」行）、Markdown/txt（UTF-8）；大小上限 `KB_MAX_FILE_SIZE_MB`（默认 10MB）。

**出题决策矩阵（D4）**：`POST /quiz` 新增可选 `knowledge_base_id`（缺省行为不变 = 自动模式）。

| 场景 | 行为 |
| --- | --- |
| 未选库（自动模式） | 检索全部知识库 → LLM 判定（`judge_knowledge_sufficient`）知识是否足够；足够 → 仅用文档资料出题；不足 → `missing_topics` 定向联网补全（先检索 KB 再补网，减少无效搜索花费） |
| 指定库（严格模式） | **永不联网**。KB 命中 → 文档资料出题；空命中 / 检索错误 / 库未配置 → 全部降级为纯输入出题（任务照常 completed，不中断用户） |
| 无知识库功能（embedding 未配置） | 上传接口 400 `KB_NOT_CONFIGURED`；出题跳过知识库段，行为等同接入前 |

**判定契约**：`KnowledgeJudgeResult{enough, reason, missing_topics≤3}`，模板 `knowledge_judge.txt`（jinja2）；出题模板 `quiz.txt` 增加【文档资料】（来自用户的私有知识库）/【参考资料】（来自联网检索）两节与【事实来源总则】（事实必须来自用户输入/文档资料/参考资料三者之一；严格模式仅前两者）。检索片段拼接为 `doc_materials` 传入生成器，检索字符数记入日志 `kb_chars=` 与现有 token 统计口径。

**配置（server/.env）**：`EMBEDDING_ENABLED`（总开关）/ `EMBEDDING_MODEL` / `EMBEDDING_API_KEY`（空则视同未启用）/ `EMBEDDING_BASE_URL` / `EMBEDDING_BATCH_SIZE`（默认 20，对齐百炼批量上限）/ `KB_MIN_SCORE` / `KB_MAX_FILE_SIZE_MB` / `KB_CHROMA_DIR`（PersistentClient 目录）。

**API**（全部 JWT 鉴权 + 归属校验，他人资源返回与「不存在」一致的 404 防枚举）：`/knowledge-base`（GET 列表含 doc_count/ready_count / POST 201 / PATCH 重命名 / DELETE 204 级联）、`/knowledge-base/{id}/document`（POST 202 + task_id 轮询 / GET 列表含解析状态）、`/knowledge-base/document/{id}`（DELETE）、`/knowledge-base/document/{id}/reindex`（POST 202）、`/knowledge-base/task/{task_id}`（状态轮询，复用 TaskStore）。

**前端**：知识库列表页 + 详情页（上传/状态徽标/重建索引/删除）；出题页选库入口（「自动（知识库+联网）」或指定库，严格模式 UI 提示「仅基于该库出题」）；`quiz_data` Storage 透传 `knowledge_base_id`。
