...
* [2. 技术选型](#-1)
  ...
  * [2.5 联网搜索选型：博查 Bocha（P1 预选型，MVP 不接入）](#BochaP1MVP)
* [3. 系统架构设计](#-1)
  * [3.1 总体架构](#-1)
...
| ❌ 不做 | 用户登录注册、数据库持久化、金币/经验值系统、**联网搜索**、多格式输入（PDF/Word/URL/视频）、RAG 私有知识库、排行榜与 PK、付费模型、服务器部署上线 |

不做登录注册与数据库的原因：MVP 阶段验证的是"AI 出题质量 + 游戏化学习体验"这一核心假设，答题与报告均为一次性即时交互，无需持久化；引入账号体系会显著拉长开发周期且不改变核心假设的验证结果。

> 决策记录（v1.1 更新）：
>
> 1. 金币/经验值系统经确认**整体延后**（含纯前端本地假数据方案），答题页只做对错判定与讲解；
...
| 联网搜索 | 博查 Bocha Web Search API（**P1 引入，MVP 不接入**） | v1 |
...
### <a name='BochaP1MVP'></a>2.5 联网搜索选型：博查 Bocha（P1 预选型，MVP 不接入）

> 决策记录：联网搜索**非 MVP 核心**。MVP 由 AI 直接基于用户输入出题（用户输入一句话/一段文本已含足够信息），联网搜索增强出题整体延后至 P1。本节为 P1 预选型结论，届时直接采用，无需重新调研。
...
**P1 接入方式**：三段式出题流水线（提取搜索词 → 搜索 → 出题），搜索失败自动降级为直接出题（不阻塞核心流程）。
...
P1 新增：`services/search`（博查客户端，三段式流水线的中间段）。
...
```text
用户输入 content
  → [LLM 步骤1] 生成 1~3 个搜索关键词
  → [博查 步骤2] 逐关键词搜索，各取 top5 的 summary 拼接（总长度截断至 ~4000 字）
  → [LLM 步骤3] 基于「用户输入 + 搜索结果」生成题库
  → 步骤 1/2 失败 → 降级为仅基于用户输入出题
```

要点：

* 温度参数：出题 `temperature=0.7`（保证题目多样性）、报告 `0.5`；
...
structured_model = model.with_structured_output(QuizSchema)  # Pydantic 模型即输出契约
quiz = structured_model.invoke(prompt_messages)              # 直接返回已校验的 QuizSchema 对象
```
...
2. 解析或校验失败 → 原请求重试 ≤2 次；仍失败 → 任务 `failed`（LLM_PARSE_FAILED），绝不把坏数据发给前端；
3. 若 DeepSeek 对某种输出约束方式（json_schema / json_mode / function_calling）兼容性不佳，切换 `with_structured_output` 的 `method` 参数即可，业务代码不动。
...
| 阶段 | 内容 | 技术要点 |
| --- | --- | --- |
| P1 | 联网搜索增强出题 | 接入博查（2.5 预选型结论），出题流水线扩展为三段式，失败降级直接出题 |
| P1 | 登录注册、答题记录持久化、历史报告 | 微信授权登录 + MySQL + 任务队列迁移 Redis/Celery |
| P1 | 多格式输入（PDF/Word/URL/视频） | 文档解析库 + 网页正文提取 |
| P1 | 后台分析复盘、艾宾浩斯错题重练 | 错题数据统计 + 订阅消息推送 |
| P2 | 好友 PK、排行榜 | 实时对战（WebSocket）+ 分领域 Elo 积分榜（规则见 6.7） |
...
### <a name='PKElo'></a>6.7 P2 PK 积分规则（预设计，已确认）

> 决策记录：排行榜**只来自 PK 对战积分**，采用 **Elo 等级分模型**，**分知识领域独立计分**；实时对战，好友邀请局为友谊赛不计分。个人闯关答题不产生榜单积分。
...
| --- | --- | --- |
| 初始分 | 1200 | 每个知识领域独立定级 |
| K 因子 | 该领域对局 <10 场 K=40，之后 K=32 | 新手期快速收敛到真实水平 |
| 期望胜率 | E = 1 / (1 + 10^((Rb − Ra)/400)) | 标准 Elo |
| 结算公式 | Ra′ = Ra + K × (S − E) | S=1 胜、0 负、0.5 平 |

#### 对局结算流程

1. 实时对战双方各自作答同一套 5 题，答对 1 题 +1 分，答完比较总分；
2. 总分高者胜（S=1）；总分平局 → 总用时短者胜；用时也相同 → S=0.5 双方各得 0.5；
3. 按双方该领域的 Elo 结算，增减写入各自记录；
4. 好友邀请局标记 `friendly=true`，完整对局体验但 Elo 不结算（防互刷刷榜）。

#### 边界与惩罚

* **中途逃跑/超时**：逃跑者判负，扣分按 K 全额（按对手 Elo 低 400 分的最坏情形计算），另加 5 分钟对战冷却——让逃跑比输更亏；
* **匹配窗口**：优先 ±300 分内对手，10 秒未匹配则逐步放宽至无限制（冷启动期用户少也能开局）；
* **段位映射**（仅展示，不影响结算）：青铜 <1100 · 白银 1100–1249 · 黄金 1250–1399 · 铂金 1400–1549 · 钻石 1550+；
* **上榜门槛**：领域榜需该领域 ≥3 场对局（防止 1 胜定榜）。

#### 算例

```text
甲（物理 Elo 1300）vs 乙（物理 Elo 1100），K=32：
E甲 = 1/(1+10^((1100−1300)/400)) ≈ 0.76
甲胜：甲 +32×(1−0.76) ≈ +8，乙 −8（胜强分多，输弱扣多）
乙爆冷胜：乙 +32×(1−0.24) ≈ +24，甲 −24（弱者爆冷收益大）
```

#### P2 数据模型

```text
领域分表：user_id, domain, elo, played, win, updated_at
PK 对局表：match_id, domain, quiz_id, 双方 user_id、得分、用时、胜负、elo 变动、friendly 标记
```

总榜 = 各领域 Elo 之和排序；分领域榜 = 该领域 Elo 排序。
...
```text
ai-learn/
├── prototype/                  # 一次性网页原型（验证后废弃，不进入生产）
│   ├── index.html              # 主页：输入 + loading
│   ├── quiz.html               # 闯关页
│   └── report.html             # 报告页
├── miniprogram/                # Taro 小程序（步骤 3 起）
│   ├── src/
│   │   ├── pages/
│   │   │   ├── index/          # 主页
│   │   │   ├── quiz/           # 闯关页
...
│   ├── app/
│   │   ├── api/                # 路由
│   │   ├── services/           # quiz / report / llm（P1: search）
│   │   ├── core/               # config / tasks / prompts / sensitive
│   │   └── models/             # Pydantic schemas（LLM 结构化输出契约）
│   ├── tests/                  # pytest 单元测试
...
* [博查 AI 开放平台](https://open.bochaai.com)（P1 引入）
