...
- 结算全程单事务（INSERT session → INSERT 逐题流水 → UPDATE 余额），任一失败整体回滚；
- 幂等：`uk_session` 唯一约束冲突时改查已存结果返回，不重复入账。
...
**错误码扩展**：`UNAUTHORIZED`（401 未登录/token 失效）、`INVALID_NICKNAME`（422 昵称非法）、`NOT_FOUND`（404）。沿用：`SENSITIVE_CONTENT`、`LLM_TIMEOUT`、`LLM_PARSE_FAILED`、`TASK_TIMEOUT`。
